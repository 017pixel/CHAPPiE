...
def test_root_config_roundtrip_keeps_cerebras_small_task_defaults():
    ...
    assert values["CEREBRAS_API_KEY"] == "test-key"
    assert values["LLM_PROVIDER"] == "vllm"
    assert values["VLLM_MODEL"] == "Qwen/Qwen3.5-4B"
    assert values["QUERY_EXTRACTION_PROVIDER"] == "cerebras"


def test_build_root_config_contains_generation_budgets():
    config = build_root_config({})

    assert config["generation"]["chappie_thinking_token_limit"] == 2500
    assert config["generation"]["chappie_answer_token_limit"] == 800
    assert config["small_tasks"]["intent_processor_model_cerebras"] == "llama-3.1-8b"


if __name__ == "__main__":
    test_root_config_roundtrip_keeps_cerebras_small_task_defaults()
