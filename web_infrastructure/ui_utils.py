...
from config.emotions import EMOTION_DEFAULTS, EMOTION_LABELS_DE, EMOTION_ORDER, clamp_emotion_value, normalize_emotion_state

T = TypeVar("T")

UI_VERSION = "13.6"

EMOTION_DISPLAY_ORDER = EMOTION_ORDER
EMOTION_LABELS = EMOTION_LABELS_DE


def _clamp_emotion_value(value, default: int) -> int:
    return clamp_emotion_value(value, default)
...
def normalize_emotions(emotions: Mapping[str, int] | None) -> dict[str, int]:
    return normalize_emotion_state(emotions)
